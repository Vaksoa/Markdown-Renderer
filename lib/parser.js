const parseMarkdown = (markdown) => {
            ...
            switch (char) {
                case '*': {
                    ...
                    if (nextChar === '*') {
                        ...
                        const boldTextEnd = text.indexOf('**', i + 2);
                        if (boldTextEnd !== -1) {
                        ...
                        const italicTextEnd = text.indexOf('*', i + 1);
                        if (italicTextEnd !== -1) {
    ...
    const handleListItem = (line, indentation) => {
        ...
        if (listItemMatch) {
            const listItemNode = createNode('list-item', {
                children: [createNode('text', { value: listItemMatch[2] })],
            });
            ...
            if (prevSibling && prevSibling.type === 'list') {
                ...
                const listNode = createNode('list', { children: [listItemNode] });
